...
namespace Titan
{
    ...
    void SceneHierarchyPanel::DrawComponents(Entity entity)
    {
        ...
        DrawComponent<TerrainRendererComponent>(
            ICON_FA_MOUNTAIN " Terrain Renderer", entity,
            [](auto& component)
            {
                    ...
                    if (ImGui::Button("Load..."))
                    {
                        ...
                        if (!filepath.empty())
                        {
                            ...
                            if (loadedMat)
                            {
                                ...
                                std::error_code ec;
                                auto relPath = std::filesystem::relative(filePath, assetDir, ec);
                                loadedMat->SetInternalPath(ec ? filePath.string() : relPath.string());
                                material = loadedMat;
                            }
                        }
                    }
                ...
                float buttonWidth = ImGui::GetContentRegionAvail().x;
                std::string matLabel = std::format(
                    "Material: {}", material->GetInternalPath().empty() ? "Unsaved" : material->GetInternalPath());
                ...
                if (ImGui::Button("Save Material"))
                {
                    if (material->GetInternalPath().empty())
                    {
                        ...
                        if (component.texture && !component.texture->GetInternalPath().empty())
                        {
                            auto texturePath = std::filesystem::path(component.texture->GetInternalPath());
                        ...
                        material->SourcePath = (Project::GetAssetDirectory() / internalPath).string();
                        std::filesystem::create_directories(Project::GetAssetDirectory() / materialDir);
                    }
                    else if (material->SourcePath.empty())
                    {
                        material->SourcePath = (Project::GetAssetDirectory() / material->GetInternalPath()).string();
                    }
