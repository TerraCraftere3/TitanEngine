...
namespace Titan
{
    Ref<Texture2D> Texture2D::Create(uint32_t width, uint32_t height, TextureFormat format, TextureSettings settings)
    {
        switch (Renderer::GetAPI())
        {
            ...
            case RendererAPI::API::OpenGL:
                return CreateRef<OpenGLTexture2D>(width, height, format, settings);
        }
    ...
    Ref<Texture2D> Texture2D::Create(void* ptr, const std::string& path)
    {
        switch (Renderer::GetAPI())
        {
            case RendererAPI::API::None:
                ...
                return nullptr;
            case RendererAPI::API::OpenGL:
                return CreateRef<OpenGLTexture2D>(ptr, path);
        }
